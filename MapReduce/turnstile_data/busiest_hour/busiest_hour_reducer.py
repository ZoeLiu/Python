...
def reducer():
    ...
    for line in sys.stdin:
        ...
        this_key, entry, date, time = data
        
        #logging.info(str(this_key) + ' ' + datetime)
        ...
        if float(entry) >= float(max_entries):
            max_entries = float(entry)
            datetime = str(date) + ' ' + str(time)
            
        #logging.info(old_key + ' ' + datetime + ' ' + entry + ' ' + max_entries)
